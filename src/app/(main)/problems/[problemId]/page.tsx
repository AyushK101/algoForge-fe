...
const Page = () => {
  ...
  const [problem, setProblem] = useState<{ title: string, description: string, testCases: Array<{ input: string, output: string }> }>();
  useEffect(() => {
    async function init() {
  ...
  return (
    <div className="flex flex-col sm:flex-row w-full h-screen px-4 sm:px-12 py-6 gap-6">
      ...
      <div className="w-full sm:w-1/2  rounded-xl shadow-md p-4 overflow-y-auto">
        <h1 className="text-2xl font-bold mb-4">{sampleProblem.title}</h1>
        <p className="text-gray-700 mb-4 whitespace-pre-wrap">{sampleProblem.description}</p>
        ...
        <div>
          ...
          <ul className="space-y-3">
            {sampleProblem.testCases.map((tc, i) => (
